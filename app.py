...
def careerFirstPage():
    ...
    return render_template("career.html", job=new_career.get("name"), desc=new_career.get("description"), salary=new_career.get("salary"), category=new_career.get("category"), degree=new_career.get("degree"), progress=career.getProgress())
...
@app.route("/favorites")
def favorites():
    return render_template("favorites.html", favorites=career.favorites)

@app.route("/resumebuilder")
def buildResume():
    choice = request.args.get("job")
    if choice != None:
        return render_template("resumebuilder.html", quizult="You picked {} from the career quiz! ".format(choice.replace("_", " ")))
    else:
        return render_template("resumebuilder.html", quizult="")
